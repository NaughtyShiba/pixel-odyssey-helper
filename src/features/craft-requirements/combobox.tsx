...
const craftableItems = Object.entries(items).filter(([,item]) => item.craft).map(([name, item]) => ({ value: name, label: item.label }));

interface CraftRequirementsComboboxProps {
  onChange(itemName: string): void;
  className?: string;
}
export function CraftRequirementsCombobox(props: CraftRequirementsComboboxProps) {
